...
pub async fn group_chat_list(
    ...
    let store = group_chat_store(&request.workspace_path)
        .await
...
pub async fn group_chat_ingest_reply(
    ...
    if !request.reply_content.trim().is_empty() {
        let reply = GroupChatMessage {
            message_id: format!("msg-reply-{}-{}", request.message_id, request.timestamp),
        ...
        store
            .append_message(&request.room_id, &reply)
            .await
            .map_err(|error| GroupChatError {
                code: group_chat_store_error_code(&error),
                message: error.to_string(),
...
pub async fn group_chat_scan_timeouts(
    ...
    let store = group_chat_store(&request.workspace_path).await?;
