...
const AgentTeamEditorView: React.FC = () => {
  ...
  const { openHome, setTeamComposerAgents, activeAgentTeamId, addMember } = useAgentsStore();
  const { allAgents, hiddenAgentIds } = useAgentsList({
    searchQuery: '',
    filterLevel: 'all',
    filterType: 'all',
    t,
  ...
  // Sync the full agent list into the team store so the gallery/composer share
  // the same real data set (no mock data in the editor).
  React.useEffect(() => {
    setTeamComposerAgents(allAgents);
  }, [allAgents, setTeamComposerAgents]);
    ...
    <div className="tv tv--editor">
      ...
      <div className="tv__body">
        <aside className="tv__gallery">
          ...
          {/* R-5 (P4): node-type palette consumes the dynamically enumerated
              agent list (useAgentsList().allAgents) — every registered agent
              type (Mode + SubAgent + custom + acp__*), hidden excluded. The
              leader slot defaults to the main Claw (= current user). Never a
              hardcoded agent list. */}
          <NodePalette
            agents={allAgents}
            hiddenAgentIds={hiddenAgentIds}
            mainClawAgentId="Claw"
            onSelect={(agentId) => {
              if (activeAgentTeamId) addMember(activeAgentTeamId, agentId);
            }}
            t={t}
          />
        </aside>
...
const AgentsHomeView: React.FC = () => {
  ...
  // Keep the team editor store in sync with the same agent list as the scene.
  React.useEffect(() => {
    setTeamComposerAgents(allAgents);
  }, [allAgents, setTeamComposerAgents]);
  ...
  return (
    ...
    >
        ...
        extraContent={(
          <div className="gallery-anchor-bar" data-bf-scene="agents" data-bf-part="anchorBar">
            ...
            <button
              type="button"
              className="gallery-anchor-btn"
              onClick={() => scrollToZone('workflow-zone')}
              data-testid="agents-anchor-workflow"
            >
              {t('nav.legions')}
            </button>
          </div>
      ...
      <div className="gallery-zones" data-bf-scene="agents" data-bf-part="zones" data-testid="agent-list">
          ...
          tools={(
            <>
              ...
              <button
                ...
                data-testid="agents-open-review-team-btn"
              >
                <ShieldCheck size={15} />
        ...
        <GalleryZone
          id="workflow-zone"
          data-testid="agents-workflow-zone"
          title={t('legionsZone.title')}
          subtitle={t('legionsZone.subtitle')}
          tools={(
            <span className="gallery-zone-count">{savedLegionPresets.length + agentTeams.length}</span>
          )}
        >
      ...
      <GalleryDetailModal
        ...
        meta={selectedTeam ? (
          <span>{t('home.members', { count: selectedTeam.members.length })}</span>
        ) : null}
          ...
          <div className="agent-team-card__section">
            ...
            <div className="agent-team-card__member-list">
              {selectedTeam.members.map((member) => {
                ...
                return (
                  <span key={member.agentId} className="agent-team-card__member-chip">
                    <AgentIcon size={11} />
                    <span className="agent-team-card__member-name">{agent?.name ?? member.agentId}</span>
        ...
        {selectedTeam ? (
          <div className="agent-team-card__section">
            ...
            <div className="agent-team-card__cap-chips">
              {CAPABILITY_CATEGORIES
                .filter((category) => computeAgentTeamCapabilities(selectedTeam, allAgents)[category] > 0)
                .map((cap) => (
                  <span
                    key={cap}
