...
fn runtime_facts_reminder_renders_time_and_offset_facts() {
    ...
    assert!(reminder.contains("当前为本地时间周3 10:30"));
    assert!(reminder.contains("DeepSeek 峰谷定价"));
}
...
fn runtime_facts_reminder_tiered_guidance_covers_high_usage_compression_and_normal() {
    // High usage (>30%): hallucination guardrail is present.
    let high = render_runtime_facts_reminder(&sample_runtime_facts_input(Some(0.35)));
    assert!(high.contains("上下文已超 30%，警惕幻觉/敷衍/自我欺骗劣根性，不确定必须查证"));
    assert!(!high.contains("即将自动压缩"));

    // At or above the dynamic compression preview point: persist warning.
    let preview = render_runtime_facts_reminder(&sample_runtime_facts_input(Some(0.9)));
    assert!(preview.contains("即将自动压缩，重要信息先落盘/总结"));
    assert!(preview.contains("上下文已超 30%"));

    // Normal usage: neither tiered line is present.
...
#[test]
fn runtime_facts_reminder_uses_conservative_preview_fallback_when_dynamic_value_is_missing() {
    ...
    assert!(reminder.contains("即将自动压缩，重要信息先落盘/总结"));
