...
pub const fn should_skip_agent_session_reply(
    ...
    // BUG-02（suppress 消息级化）：注入抑制判定已上移到
    // `resolve_agent_session_reply_action`（agent-runtime/scheduler.rs）——按
    // 目标 turn 是否承载 normal 回传义务（AgentSession 发起 + reply_route 存在）
    // 区分：纯 urgent 注入 turn（无义务）→ SkipSuppressedInjectedReply；有 normal
    // 义务的 turn（同 turn normal+urgent 混合）→ 注入抑制不生效，仍 Forward。
    // 本函数只保留 cancelled/retired 语义。
