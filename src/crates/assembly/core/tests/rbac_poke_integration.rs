...
fn penalty_execution_l3_is_reminder_only() {
    ...
    // Executor template has no ReadOnly class; a read-only freeze would ADD
    // ReadOnly. Read staying denied proves no freeze patch was applied.
    let read_result = post_restrictions.ensure_operation_allowed(OperationClass::ReadOnly, "Read");
    assert!(
        read_result.is_err(),
        "R-25: Read stays denied exactly as before the penalty (no read-only freeze)"
    );
