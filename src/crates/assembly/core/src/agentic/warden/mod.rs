...
//!
//! Per `phase-rbac-poke-type-contract.md`, the Poke DTOs live in
...
// ---------------------------------------------------------------------------
// 5. Penalty System (§5 of phase-rbac-poke-type-contract)
// ---------------------------------------------------------------------------

/// Penalty severity level.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
pub enum PenaltyLevel {
    /// First minor violation: shame-wall record + inject_context (<100 tokens).
    L1,
    /// Second violation in same session: RBAC demotion + shame-wall + context.
    L2,
    /// ≥3 violations or severe: RBAC read-only + session freeze + notify user.
    L3,
    /// Cross-session ≥5 violations: permanent mark + initial RBAC pre-demotion.
    L4,
}
...
// ---------------------------------------------------------------------------
// 6. Shame Wall Persistence (§6 of phase-rbac-poke-type-contract)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 7. Bootstrap / Reminder Kind Constants (§7 of phase-rbac-poke-type-contract)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Shame-wall file path constant (§6 of phase-rbac-poke-type-contract)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 8. Poke-First Protocol (§8 of phase-rbac-poke-type-contract)
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn shame_wall_upsert_existing_entry() {
        ...
        registry.upsert_entry("user-1", "executor", "session-1", vec![v1], PenaltyLevel::L1, "t1");
        ...
        registry.upsert_entry("user-1", "executor", "session-1", vec![v2], PenaltyLevel::L2, "t2");

        assert_eq!(registry.entries.len(), 1);
        assert_eq!(registry.entries[0].violations.len(), 2);
        assert_eq!(registry.entries[0].cumulative_penalty_level, PenaltyLevel::L2);
    }
    ...
    fn poke_message_example() {
        let json = r#"{
            "poke_id": "poke-abc-123",
            "poke_type": "Challenge",
            "rule_ids": ["R-001", "R-002"],
            "deadline_turns": 5,
            "evidence_required": ["tool-call-log", "phase-summary"]
        }"#;
    ...
    fn poke_response_example() {
        let json = r#"{
            ...
            "self_check": {
                ...
                "tool_calls_summary": ["Read(main.rs)", "Edit(main.rs:42)"],
                "rules_checked": ["R-001", "R-004"]
            }
        }"#;
