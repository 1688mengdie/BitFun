...
fn build_default_role_permissions() -> RolePermissionMap {
    ...
    // ── Reviewer ───────────────────────────────────────────────────
    // Allowed operation classes: WriteFile + ExecuteCode (≈ Executor).
...
mod tests {
    ...
    fn reviewer_gets_writefile_and_executecode_like_executor() {
        ...
        // ReadOnly is NOT in the Reviewer default set: reviewers inspect and
        // reproduce findings (≈ Executor), they are not read-only shells.
        assert!(
            !permissions
                .allowed_operation_classes
                .contains(&OperationClass::ReadOnly),
            "Reviewer should not allow ReadOnly (not in default allowed set)"
        );
    }
