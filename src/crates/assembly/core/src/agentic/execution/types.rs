...
pub struct RoundContext {
    ...
    /// BEFORE dynamic MCP merge). The runtime RBAC gate unions this with the
    /// role template whitelist so front-end checked tools execute (勾选=权威);
    /// unchecked tools stay blocked even when visible.
...
impl RoundResult {
    /// A zero-cost fallback used when the engine decides not to spend another
    /// model request (for example when the finalize budget is exhausted). It
    /// carries no usable assistant text, so callers fall through to local
    /// final-response synthesis without issuing another provider call.
...
pub struct ExecutionResult {
    ...
    /// Total number of tool calls executed during this execution
    pub total_tools: usize,
    /// Total token usage reported by the model for this execution (0 when unavailable)
    pub total_tokens: usize,
    /// Total wall-clock duration of this execution in milliseconds
    pub duration_ms: u64,
    pub success: bool,
