...
///
/// # Deliberate ordering (d1-P2-6)
///
/// The `force_requested` rejection is evaluated **before** the "no active
/// constraint" fast path: a stale caller that sends `force` is denied even
/// when no constraint is currently enforceable. This is an intentional
/// tightening — `force` is never a valid input any more, so it is not silently
/// absorbed by the early-return path that allows ordinary (force-free) calls
/// to proceed. Treating `force` as a hard 403 keeps every legacy call site
/// observable instead of quietly downgrading them to the permissive branch.
/// Do not reorder these two branches without revisiting this contract.
pub fn check(
    context: Option<&ToolUseContext>,
    tool_name: &str,
    operation: &str,
